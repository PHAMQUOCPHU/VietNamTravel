...
const AdvancedSearch = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 mt-8 md:-mt-10 relative z-50">
      <div className="bg-white/95 backdrop-blur-md md:rounded-full rounded-3xl shadow-[0_22px_55px_rgba(30,58,138,0.18)] p-2 border border-blue-100 flex flex-col md:flex-row items-center divide-y md:divide-y-0 md:divide-x divide-gray-100 dark:bg-slate-900/95 dark:border-slate-700 dark:divide-slate-700 w-full">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-sm font-extrabold text-slate-800 dark:text-slate-100 truncate">
                {searchData.province || regionDisplayLabel || "Tìm kiếm địa điểm"}
              </span>
            </div>
          </div>
          <AnimatePresence>
            {showDest && (
              <motion.div
                ...
                className="absolute top-full left-0 mt-3 w-72 bg-white dark:bg-slate-900 shadow-2xl rounded-2xl p-4 border border-blue-100 dark:border-slate-600 z-50 flex flex-col gap-4"
                onClick={(e) => e.stopPropagation()}
              >
                <div>
                  <p className="text-xs font-bold text-slate-500 mb-2 uppercase tracking-wider">Vùng miền</p>
                  <div className="grid grid-cols-2 gap-2">
                    {REGION_OPTIONS.map((o) => (
                      <button
                        key={o.value || "all"}
                        type="button"
                        onClick={() => setSearchData((prev) => ({ ...prev, region: o.value }))}
                        className={`text-left py-2 px-3 rounded-lg text-xs font-bold transition-colors ${searchData.region === o.value ? "bg-blue-600 text-white" : "bg-slate-50 text-slate-700 hover:bg-blue-50"}`}
                      >
                        {o.label}
                      </button>
                    ))}
                ...
                <div>
                  <p className="text-xs font-bold text-slate-500 mb-2 uppercase tracking-wider">Tỉnh thành</p>
                  <select
                    className="w-full bg-slate-50 dark:bg-slate-800 text-sm font-bold text-slate-700 dark:text-slate-200 outline-none p-2.5 rounded-xl border border-slate-200 dark:border-slate-700"
                    value={searchData.province}
                    onChange={(e) => {
                      setSearchData((prev) => ({ ...prev, province: e.target.value }));
                      setShowDest(false);
                    }}
                  >
            ...
            <div className="flex flex-col min-w-0 w-full">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="bg-transparent text-sm font-extrabold text-slate-800 dark:text-slate-100 outline-none w-full max-w-[110px] cursor-pointer"
                  onChange={(e) =>
                    setSearchData((prev) => ({
                      ...prev,
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex gap-2 items-center">
                      <button
                        type="button"
                        onClick={() => setSearchData((prev) => ({ ...prev, adults: Math.max(1, prev.adults - 1) }))}
                        className="w-8 h-8 border border-slate-200 rounded-full flex justify-center items-center font-bold hover:border-slate-800"
                      >-</button>
                      <span className="text-sm font-bold w-4 text-center">{searchData.adults}</span>
                      <button
                        type="button"
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex gap-2 items-center">
                      <button
                        type="button"
                        onClick={() => setSearchData((prev) => ({ ...prev, children: Math.max(0, prev.children - 1) }))}
                        className="w-8 h-8 border border-slate-200 rounded-full flex justify-center items-center font-bold hover:border-slate-800"
                      >-</button>
                      <span className="text-sm font-bold w-4 text-center">{searchData.children}</span>
                      <button
                        type="button"
