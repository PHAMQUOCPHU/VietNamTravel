...
export const createDiary = async (req, res) => {
  try {
    const { userId, bookingId, tourId, tourTitle, title, content, rating, emotion, location, travelDate } = req.body;
    
    // Check if a diary already exists for this booking
    const existingDiary = await diaryModel.findOne({ bookingId });
    if (existingDiary) {
      return res.status(400).json({ success: false, message: "Bạn đã viết nhật ký cho chuyến đi này rồi." });
    }
...
export const getEligibleBookings = async (req, res) => {
  try {
    const { userId } = req.body;
    // Lấy các booking đã completed
    const completedBookings = await bookingModel.find({ userId, status: "completed" }).sort({ createdAt: -1 });
    
    // Lọc những booking chưa được viết nhật ký
    const writtenDiaries = await diaryModel.find({ userId }).select('bookingId');
    const writtenBookingIds = writtenDiaries.map(d => d.bookingId.toString());

    const eligibleBookings = completedBookings.filter(b => !writtenBookingIds.includes(b._id.toString()));
